            ...
            <?php  
                ...
                for($cb=1;$cb<=$boxDisplay;$cb++){ 
                    if(($cb >= $currentMonthFirstDay || $currentMonthFirstDay == 1) && $cb <= ($totalDaysOfMonthDisplay)){ 
                        ...
                        global $mysqli; 
                        $result = $mysqli->query("SELECT title FROM events WHERE date = '".$currentDate."' AND status = 1"); 
...
function getEvents($date = ''){ 
    ...
    global $mysqli; 
    $result = $mysqli->query("SELECT title FROM events WHERE date = '".$date."' AND status = 1"); 
...
function addEvent($postData){ 
    ...
    if(!empty($postData['event_title']) && !empty($postData['event_date'])){ 
        global $mysqli; 
        $event_title = $mysqli->real_escape_string($postData['event_title']); 
        $insert = $mysqli->query("INSERT INTO events (title, date) VALUES ('".$event_title."', '".$postData['event_date']."')"); 
        if($insert){ 
            $status = 1; 
        } 
    } 
